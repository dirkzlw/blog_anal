...
/**
  ...
  */
object AgeLogisticRegression_2 {
  def main(args: Array[String]): Unit = {
    // 构建Spark对象，本地执行
    val conf = new SparkConf().setAppName("AgeLogisticRegression_2").setMaster("local")
    val sc = new SparkContext(conf)

    val data: RDD[regression.LabeledPoint] = MLUtils.loadLibSVMFile(sc, "./data/age2.data")
